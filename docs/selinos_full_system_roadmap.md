...
|---|---|---|---|
| **P0** | Phase 78 M1 fresh stack data-access translation witness. | One bounded fresh-target data-access observation with explicit fault classification or access witness, mapping provenance, and no ELF/process claim. | General stack, ABI entry state or Linux compatibility. |
...
The current baseline records **84 standalone verified probes** and a successful **51-profile rebuild** after Phase 78 M0. This is meaningful progress in evidence infrastructure and primitive OS services: after terminal revocation, target-CNode deletion, root-held disposal and status-only fresh-generation authorization, one independently configured fresh target has completed one NOP, received one RIP-only reply that skips its first deliberate `UD2`, completed a second NOP, and reached a second terminal `UD2` fault under a fresh RX-entry/NX-stack mapping. Phase 78 M0 additionally records the expected suspended mapping ledger: the fresh stack map reuses the Phase 74 IPC hierarchy and the fresh entry map allocates one final page-table object. It is **not** a bootable Debian/Ubuntu replacement and it does **not** yet run `apt`, `dpkg`, arbitrary Linux packages, general Linux applications or arbitrary Linux C drivers. The immediate technical blocker is a bounded fresh target data-access translation witness; larger blockers remain W^X ELF runtime, durable storage under a valid DMA policy, networking/TLS/time and the package transaction stack.
