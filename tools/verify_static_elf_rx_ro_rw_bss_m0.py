#!/usr/bin/env python3
"""Independently verify the bounded Phase 81 ELF64 RX/RO/RW+BSS M0 proof."""
...
MANIFEST = ROOT / "tests/artifacts/selinos_static_elf_rx_ro_rw_bss_m0.verification.json"
...
def main() -> int:
    ...
    require(evidence.get("milestone") == "SeLinOS static ELF64 RX plus RO plus RW/BSS load M0",
            "wrong milestone")
    profile = evidence["profile"]
    require(profile["cmake_option"] == "SeLinStaticElfRxRoRwBssM0=ON",
    ...
    require("#define CONFIG_SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_PROBE" in config and
            "/* disabled: CONFIG_SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_PROBE */" not in config,
            "Phase 81 selector not enabled in isolated generated config")

    sources = {label: text(record, label)
               for label, record in evidence["implementation"].items()}
    cmake = sources["build_gate"]
    for required in (
        "SeLinStaticElfRxRoRwBssM0",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_PROBE",
        "DEFAULT\n    OFF",
        "selinos-static-elf-rx-ro-rw-bss-m0-fixture",
        "static_elf_rx_ro_rw_bss_m0_root.c",
    ):
        require(required in cmake, f"missing Phase 81 CMake contract: {required}")
    ...
    for required in (
        ...
        "[176] = 1u",
        "[180] = 6u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_RO_INITIALIZED_BYTES",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_DATA_INITIALIZED_BYTES",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_DATA_MEMORY_BYTES",
        "summary->load_segments != 3u",
        ...
        "static_elf_rx_ro_rw_bss_fixture",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_BSS_WRITE_VALUE",
        "[SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_TEXT_LOAD_OFFSET + 42u] = 0x4fu",
    ):
        require(required in fixture, f"missing fixed three-segment fixture/parser guard: {required}")
    ...
    for required in (
        ...
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_ENTRY_VADDR",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_RO_VADDR",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_DATA_VADDR",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_BSS_OFFSET",
        "seL4_X86_ExecuteDisable",
        "seL4_TCB_Resume(target_tcb.cptr)",
        "seL4_Fault_UserException",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_SUCCESS_UD2_OFFSET",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_INVALID_OPCODE_VECTOR",
    ):
    ...
    target_text_map = transaction.index("(void *)SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_ENTRY_VADDR")
    target_ro_map = transaction.index("(void *)SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_RO_VADDR")
    target_data_map = transaction.index("(void *)SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_DATA_VADDR")
    require(text_unmap < target_text_map and ro_unmap < target_ro_map and
            data_unmap < target_data_map,
            "root writable aliases are not unmapped before target mappings")

    protocol = sources["protocol"]
    for required in (
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FAULT_BADGE 0x81u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_ENTRY_VADDR 0x60000000u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_DATA_VADDR 0x60001000u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_FIXED_RO_VADDR 0x60002000u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_TEXT_PAYLOAD_BYTES 72u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_RO_INITIALIZED_BYTES 4u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_DATA_INITIALIZED_BYTES 4u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_DATA_MEMORY_BYTES 64u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_BSS_BYTES 60u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_DATA_INITIAL_VALUE 0x534c3831u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_RO_INITIAL_VALUE 0x524f3831u",
        "SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_SUCCESS_UD2_OFFSET 70u",
    ):
    ...
    print("SeLinOS static ELF64 RX plus RO plus RW/BSS load M0 evidence verified.")
    return 0
