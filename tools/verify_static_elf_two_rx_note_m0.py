#!/usr/bin/env python3
"""Independently verify the bounded Phase 84 two-RX ELF64 M0 proof."""
...
MANIFEST = ROOT / "tests/artifacts/selinos_static_elf_two_rx_note_m0.verification.json"
...
def main() -> int:
    ...
    require(evidence.get("milestone") ==
            "SeLinOS static ELF64 two-RX plus PT_NOTE load M0",
            "wrong milestone")
    require(evidence.get("status") == "verified", "milestone is not verified")

    profile = evidence["profile"]
    require(profile["cmake_option"] == "SeLinStaticElfTwoRxNoteM0=ON",
    ...
    require(profile["note"] == "exactly one PF_R PT_NOTE, p_vaddr=p_paddr=0, p_filesz=p_memsz=4 at file offset 0x200, not mapped",
            "wrong bounded PT_NOTE contract")
    ...
    require("#define CONFIG_SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_PROBE" in config and
            "/* disabled: CONFIG_SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_PROBE */" not in config,
            "Phase 84 selector not enabled in isolated generated config")

    sources = {label: text(record, label)
               for label, record in evidence["implementation"].items()}
    cmake = sources["build_gate"]
    for required in (
        "SeLinStaticElfTwoRxNoteM0",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_PROBE",
        "DEFAULT\n    OFF",
        "selinos-static-elf-two-rx-note-m0-fixture",
        "static_elf_two_rx_note_m0_root.c",
    ):
        require(required in cmake, f"missing Phase 84 CMake contract: {required}")
    ...
    for required in (
        ...
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_ENTRY_PAYLOAD_BYTES",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_SECOND_RX_PAYLOAD_BYTES",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_RO_INITIALIZED_BYTES",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_DATA_MEMORY_BYTES",
    ...
    for required in (
        ...
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_ENTRY_VADDR",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_SECOND_RX_VADDR",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_RO_VADDR",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_DATA_VADDR",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_BSS_OFFSET",
        "selinos_static_elf_two_rx_note_m0_validate_fixture",
        "seL4_X86_ExecuteDisable",
        "seL4_TCB_Resume(target_tcb.cptr)",
        "seL4_Fault_UserException",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_SUCCESS_UD2_OFFSET",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_INVALID_OPCODE_VECTOR",
    ):
    ...
    require("NOTE_LOAD_OFFSET" not in transaction and "NOTE_BYTES" not in transaction,
            "PT_NOTE payload must not enter root mapping transaction")
    ...
    target_entry_map = transaction.index(
        "(void *)SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_ENTRY_VADDR")
    target_second_map = transaction.index(
        "(void *)SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_SECOND_RX_VADDR")
    target_ro_map = transaction.index(
        "(void *)SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_RO_VADDR")
    target_data_map = transaction.index(
        "(void *)SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_DATA_VADDR")
    ...
    require(transaction.index(
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_SECOND_RX_VADDR", fault_check) >
            fault_check,
            "terminal fault is not bound to the second RX page")

    protocol = sources["protocol"]
    for required in (
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FAULT_BADGE 0x84u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_ENTRY_VADDR 0x60000000u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_DATA_VADDR 0x60001000u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_RO_VADDR 0x60002000u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_FIXED_SECOND_RX_VADDR 0x60003000u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_NOTE_LOAD_OFFSET 512u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_NOTE_BYTES 4u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_ENTRY_PAYLOAD_BYTES 5u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_SECOND_RX_PAYLOAD_BYTES 72u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_RO_INITIALIZED_BYTES 4u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_DATA_INITIALIZED_BYTES 4u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_DATA_MEMORY_BYTES 64u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_BSS_BYTES 60u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_DATA_INITIAL_VALUE 0x534c3832u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_RO_INITIAL_VALUE 0x524f3832u",
        "SELINOS_STATIC_ELF_TWO_RX_NOTE_M0_SUCCESS_UD2_OFFSET 70u",
    ):
    ...
    print("SeLinOS static ELF64 two-RX plus PT_NOTE load M0 evidence verified.")
    return 0
