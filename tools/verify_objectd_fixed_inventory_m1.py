...
def main() -> int:
    ...
    record = json.loads(
        (root / "tests/artifacts/selinos_objectd_fixed_inventory_m1.verification.json").read_text()
    )
    need(record["schema"] == 1, "invalid objectd M1 evidence schema")
    need(record["platform"]["linux_kernel_present"] is False, "invalid objectd M1 platform evidence")
    ...
    for required in (
        "SELINOS_OBJECTD_M1_RESERVE",
        "SELINOS_OBJECTD_M1_RESERVED",
        "SELINOS_OBJECTD_M1_EBUSY",
        "SELINOS_OBJECTD_M1_EINVAL",
        "SELINOS_OBJECTD_M1_SLOT 1u",
        "SELINOS_OBJECTD_M1_ENDPOINT_SLOT 8u",
    ):
        need(required in protocol, f"missing protocol control: {required}")

    for required in (
        "start_objectd_fixed_inventory_m1_bundle",
        "selinos-objectd-m1-probe",
        "vka_alloc_endpoint(vka, &endpoint)",
        "objectd_slot != SELINOS_OBJECTD_M1_ENDPOINT_SLOT",
        "probe_slot != SELINOS_OBJECTD_M1_PROBE_ENDPOINT_SLOT",
    ):
        need(required in root_wiring, f"missing root wiring control: {required}")

    for required in (
        "request_is_exact_reserve",
        "bool reserved = false",
        "reply_status(SELINOS_OBJECTD_M1_RESERVED)",
        "reply_status(SELINOS_OBJECTD_M1_EBUSY)",
        "fixed inventory reservation-state proof passed",
    ):
        need(required in objectd, f"missing objectd control: {required}")
    for forbidden in (
        "vka_", "vspace_", "sel4utils_configure_process", "seL4_Untyped_Retype",
        "seL4_TCB_", "seL4_IRQControl", "seL4_X86_IOPort", "seL4_X86_IOSpace",
        "SELINOS_LINUX_CLONE",
    ):
        need(forbidden not in objectd, f"forbidden objectd authority/control: {forbidden}")

    for required in (
        "call_reserve_expect(SELINOS_OBJECTD_M1_RESERVED)",
        "call_reserve_expect(SELINOS_OBJECTD_M1_EBUSY)",
        "one-slot reserve then EBUSY passed",
    ):
    ...
    for excluded in ("TCB", "capability lease", "Linux clone", "dpkg or apt"):
        need(excluded in claims, f"missing non-claim: {excluded}")

    print("SeLinOS objectd fixed inventory M1 evidence verified.")
    return 0
