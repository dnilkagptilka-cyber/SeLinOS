...
"""Independently verify SeLinOS restricted Linux syscall ABI M2 evidence."""
...
BUILD = ROOT / "build"
RECORD = ROOT / "tests/artifacts/selinos_linux_syscall_abi_m2.verification.json"
LOG = ROOT / "tests/artifacts/selinos_linux_syscall_abi_m2.boot.log"
...
def main() -> int:
    ...
    require("syscall" in probe_source and "selinos_linux_write_message" in probe_source,
            "probe must issue Linux-shaped syscall and carry a write payload")
    require("sel4utils_dup_and_map" in gateway_source,
            "gateway must temporarily map a bounded user page")
    require("SELINOS_ABI_MAX_WRITE_BYTES 127u" in gateway_source,
            "gateway must bound writes to 127 bytes")
    require("seL4_UnknownSyscall_RSI" in gateway_source and
            "seL4_UnknownSyscall_RDX" in gateway_source,
            "gateway must authenticate write buffer pointer and length")
    require("SELINOS_LINUX_EXIT" in gateway_source,
            "gateway must validate exit syscall")
    ...
    require(output.index(markers[0]) < output.index(markers[1]) < output.index(markers[2]),
            "write payload, gateway completion and normal bootstrap must be ordered")
    print("SeLinOS Linux syscall ABI M2 evidence verified.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Linux syscall ABI M2 verification failed: {error}", file=sys.stderr)
        raise SystemExit(1)
