...
bool selinos_static_elf_rx_load_m0_start(vka_t *vka, vspace_t *vspace)
{
    ...
    _Static_assert(sizeof(seL4_UserContext) / sizeof(seL4_Word) ==
                       SELINOS_STATIC_ELF_RX_LOAD_M0_X86_64_CONTEXT_WORDS,
                   "Phase 79 requires complete pinned x86_64 context");
    if (vka == NULL || vspace == NULL) {
        return false;
    }
    image = selinos_static_elf_rx_load_m0_fixture(&image_bytes);
    if (selinos_static_elf_rx_load_m0_validate_fixture(image, image_bytes,
    ...
    if (vka_alloc_endpoint(vka, &fault_endpoint) != seL4_NoError ||
        ...
        vka_alloc_cnode_object(vka, SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
        ...
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_CNODE_SELF_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_cnode.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_NOTIFICATION_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_notification.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_IPC_FRAME_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_ipc_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Mint(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_FAULT_ENDPOINT_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, fault_endpoint.cptr,
                        seL4_WordBits, seL4_AllRights,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_FAULT_BADGE) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_ENTRY_FRAME_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_entry_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_STACK_FRAME_SLOT,
                        SELINOS_STATIC_ELF_RX_LOAD_M0_CNODE_SLOT_BITS,
    ...
    for (index = 0u; index < SELINOS_STATIC_ELF_RX_LOAD_M0_PAGE_BYTES; ++index) {
        ((volatile uint8_t *)root_entry_mapping)[index] = 0u;
    }
    for (index = 0u; index < SELINOS_STATIC_ELF_RX_LOAD_M0_PAYLOAD_BYTES; ++index) {
        ((volatile uint8_t *)root_entry_mapping)[index] =
            image[SELINOS_STATIC_ELF_RX_LOAD_M0_LOAD_OFFSET + index];
    }
    ...
    if (sel4utils_map_page(vka, target_vspace_root.cptr, target_ipc_frame.cptr,
                           (void *)SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_IPC_BUFFER_VADDR,
        ...
        sel4utils_map_page_with_attributes(
            ...
            (void *)SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_STACK_VADDR,
            read_write,
        ...
        sel4utils_map_page_with_attributes(
            ...
            (void *)SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_ENTRY_VADDR,
            read_only, seL4_X86_Default_VMAttributes,
            paging_objects, &paging_object_count) != seL4_NoError ||
        seL4_TCB_Configure(target_tcb.cptr,
                           SELINOS_STATIC_ELF_RX_LOAD_M0_TARGET_FAULT_ENDPOINT_SLOT,
                           target_cnode.cptr, 0u, target_vspace_root.cptr, 0u,
                           SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_IPC_BUFFER_VADDR,
                           target_ipc_frame.cptr) != seL4_NoError) {
        return false;
    }

    requested_context.rip = SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_ENTRY_VADDR;
    requested_context.rsp = SELINOS_STATIC_ELF_RX_LOAD_M0_FIXED_STACK_POINTER;
    if (seL4_TCB_WriteRegisters(target_tcb.cptr, 0u, 0u,
        ...
        ((const seL4_Word *)&observed_context)
            [SELINOS_STATIC_ELF_RX_LOAD_M0_X86_64_RFLAGS_WORD] !=
                SELINOS_STATIC_ELF_RX_LOAD_M0_X86_64_NORMALIZED_RFLAGS ||
