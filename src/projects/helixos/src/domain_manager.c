...
#endif
#if CONFIG_SELINOS_STATIC_ELF_RX_RW_BSS_M0_PROBE
#include "selinos_static_elf_rx_rw_bss_m0.h"
#endif
#if CONFIG_SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_PROBE
#include "selinos_static_elf_rx_ro_rw_bss_m0.h"
#endif
...
static bool start_sealed_static_image_mapping_m0(vka_t *vka, vspace_t *vspace)
{
    ...
    requested_context.rsp = SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_M0_FIXED_RSP;
    if (seL4_TCB_WriteRegisters(fresh_tcb.cptr, 0u, 0u,
                                SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_M0_CONTEXT_WORDS,
                                &requested_context) != seL4_NoError ||
        seL4_TCB_ReadRegisters(fresh_tcb.cptr, 0u, 0u,
                               SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_M0_CONTEXT_WORDS,
                               &observed_context) != seL4_NoError ||
    ...
    if (SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_M0_FIXED_RIP !=
        ...
        0x74u != SELINOS_FRESH_TARGET_STACK_DATA_ACCESS_M1_FAULT_BADGE) {
        return false;
    }
    fresh_entry_root_mapping = vspace_map_pages(vspace, &fresh_entry_frame.cptr, NULL,
                                                seL4_AllRights, 1u, seL4_PageBits, 1u);
    if (fresh_entry_root_mapping == NULL) {
        return false;
    }
    ((volatile uint8_t *)fresh_entry_root_mapping)[0] = 0x48u;
    ((volatile uint8_t *)fresh_entry_root_mapping)[1] = 0x8bu;
    ((volatile uint8_t *)fresh_entry_root_mapping)[2] = 0x04u;
    ((volatile uint8_t *)fresh_entry_root_mapping)[3] = 0x24u;
    ((volatile uint8_t *)fresh_entry_root_mapping)[4] = 0x0fu;
    ((volatile uint8_t *)fresh_entry_root_mapping)[5] = 0x0bu;
    vspace_unmap_pages(vspace, fresh_entry_root_mapping, 1u, seL4_PageBits,
                       VSPACE_PRESERVE);
    ...
    if (fault_badge != SELINOS_FRESH_TARGET_STACK_DATA_ACCESS_M1_FAULT_BADGE) {
        return false;
    }
    if (seL4_MessageInfo_get_label(fault_message) == seL4_Fault_UserException) {
        if (seL4_GetMR(seL4_UserException_FaultIP) !=
                SELINOS_FRESH_TARGET_STACK_DATA_ACCESS_M1_POST_READ_UD2_VADDR ||
            seL4_GetMR(seL4_UserException_SP) !=
                SELINOS_FRESH_TARGET_STACK_DATA_ACCESS_M1_FIXED_STACK_POINTER ||
            seL4_GetMR(seL4_UserException_Number) !=
                SELINOS_FRESH_TARGET_STACK_DATA_ACCESS_M1_INVALID_OPCODE_VECTOR) {
            return false;
        }
        debug_puts("SeLinOS fresh target stack data-access M1: one mov rax,[rsp] completed then terminal invalid-opcode fault received; no reply or second resume.\n");
    } else if (seL4_MessageInfo_get_label(fault_message) == seL4_Fault_VMFault) {
        ...
        debug_puts("SeLinOS fresh target stack data-access M1: initial rsp data read produced classified VMFault; no reply, repair or second resume.\n");
    } else {
        return false;
    }
#endif
...
fresh_stack_fsr_m2_done:
    ...
    if (SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_M0_FIXED_RIP !=
        ...
        0x74u != SELINOS_FRESH_TARGET_FIRST_FETCH_M0_FAULT_BADGE) {
        return false;
    }
    fresh_entry_root_mapping = vspace_map_pages(vspace, &fresh_entry_frame.cptr, NULL,
                                                seL4_AllRights, 1u, seL4_PageBits, 1u);
    ...
    if (sel4utils_map_page_with_attributes(
            vka, fresh_vspace_root.cptr, fresh_entry_frame.cptr,
            (void *)SELINOS_FRESH_TARGET_FIRST_FETCH_M0_FIXED_ENTRY_VADDR,
            seL4_AllRights, seL4_X86_Default_VMAttributes, fresh_entry_paging_objects,
            &fresh_entry_paging_object_count) != seL4_NoError ||
        sel4utils_map_page_with_attributes(
            vka, fresh_vspace_root.cptr, fresh_stack_frame.cptr,
            (void *)SELINOS_FRESH_TARGET_FIRST_FETCH_M0_FIXED_STACK_VADDR,
            seL4_AllRights,
            (seL4_X86_VMAttributes)(seL4_X86_Default_VMAttributes |
                                     seL4_X86_ExecuteDisable),
            fresh_stack_paging_objects, &fresh_stack_paging_object_count) != seL4_NoError ||
        seL4_TCB_Resume(fresh_tcb.cptr) != seL4_NoError) {
        return false;
    }
    fault_badge = 0u;
    fault_message = seL4_Recv(fresh_fault_endpoint.cptr, &fault_badge);
    if (seL4_MessageInfo_get_label(fault_message) != seL4_Fault_UserException ||
        fault_badge != SELINOS_FRESH_TARGET_FIRST_FETCH_M0_FAULT_BADGE ||
        seL4_GetMR(seL4_UserException_FaultIP) !=
    ...
    debug_puts("SeLinOS fresh target first-fetch M0: one NOP completed then terminal invalid-opcode fault received; no fault reply or second resume.\n");
#endif
#if CONFIG_SELINOS_FRESH_TARGET_FAULT_REPLY_RESTART_PROBE
    if (SELINOS_FRESH_TARGET_FIRST_FETCH_M0_FIXED_POST_NOP_VADDR !=
    ...
    fault_message = seL4_Recv(fresh_fault_endpoint.cptr, &fault_badge);
    if (seL4_MessageInfo_get_label(fault_message) != seL4_Fault_UserException ||
        fault_badge != SELINOS_FRESH_TARGET_FAULT_REPLY_RESTART_M0_FAULT_BADGE ||
        seL4_GetMR(seL4_UserException_FaultIP) !=
    ...
    debug_puts("SeLinOS fresh target fault reply/restart M0: first UserException RIP repaired to one second NOP, then terminal invalid-opcode fault received; no second reply or resume.\n");
#endif
    return true;
}
#endif
...
#endif
#if CONFIG_SELINOS_STATIC_ELF_RX_RW_BSS_M0_PROBE
    if (!selinos_static_elf_rx_rw_bss_m0_start(vka, vspace)) {
        debug_puts("SeLinOS static ELF RX/RW+BSS M0: fixed ELF validation, root-private initialization, RX/RW+NX mapping or terminal UD2 witness failed.\n");
        return false;
    }
    debug_puts("SeLinOS static ELF RX/RW+BSS M0: fixed ELF64 ET_EXEC RX text plus separate RW BSS PT_LOAD validated and copied through root-private aliases before target W^X mappings.\n");
    debug_puts("SeLinOS static ELF RX/RW+BSS M0: initialized data and one zero BSS byte were read, one BSS byte was written and reread, then terminal UD2 was received without reply, repair, retry or second resume.\n");
    debug_puts("SeLinOS static ELF RX/RW+BSS M0: no relocation, dynamic linker, C runtime, syscall, process or Linux ABI claim.\n");
#endif
#if CONFIG_SELINOS_STATIC_ELF_RX_RO_RW_BSS_M0_PROBE
    if (!selinos_static_elf_rx_ro_rw_bss_m0_start(vka, vspace)) {
        debug_puts("SeLinOS static ELF RX/RO/RW+BSS M0: fixed ELF validation, root-private initialization, RX/RO+NX/RW+NX mapping or terminal UD2 witness failed.\n");
        return false;
    }
    debug_puts("SeLinOS static ELF RX/RO/RW+BSS M0: fixed ELF64 ET_EXEC RX text, separate RO PT_LOAD and separate RW BSS PT_LOAD validated and copied through root-private aliases before target W^X mappings.\n");
    debug_puts("SeLinOS static ELF RX/RO/RW+BSS M0: initialized RW and RO values plus zero BSS were read, one BSS byte was written and reread, then terminal UD2 was received without reply, repair, retry or second resume.\n");
    debug_puts("SeLinOS static ELF RX/RO/RW+BSS M0: no relocation, dynamic linker, C runtime, syscall, process or Linux ABI claim.\n");
#endif
#if CONFIG_SELINOS_STATIC_ELF_TWO_RX_RO_RW_BSS_M0_PROBE
    if (!selinos_static_elf_two_rx_ro_rw_bss_m0_start(vka, vspace)) {
        debug_puts("SeLinOS static ELF two-RX/RO/RW+BSS M0: fixed ELF validation, root-private initialization, RX/RX/RO+NX/RW+NX mapping or terminal second-RX UD2 witness failed.\n");
        return false;
    }
...
#endif
#if CONFIG_SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_PROBE
    if (!selinos_controlled_et_dyn_relative_m0_start(vka, vspace)) {
        debug_puts("SeLinOS controlled ET_DYN relative M0: fixed ET_DYN validation, RELATIVE relocation, W^X mapping or terminal second-RX UD2 witness failed.\n");
        return false;
    }
...
#endif
#if CONFIG_SELINOS_EXECVE_REPLY_ARGV0_STRING_BYTE_M0_PROBE
    if (!selinos_execve_reply_argv0_string_byte_m0_start(vka, vspace)) {
        debug_puts("SeLinOS execve reply argv0 string-byte M0: context bridge, one argv0 byte read or terminal witness failed.\n");
        return false;
    }
...
#endif
#if CONFIG_SELINOS_FRESH_TARGET_CONTEXT_PROVENANCE_PROBE
    debug_puts("SeLinOS fresh target context provenance M0: one suspended context write-readback; no resume, execution or Linux-process claim.\n");
#endif
#if CONFIG_SELINOS_FRESH_TARGET_FAULT_REPLY_RESTART_PROBE
