...
bool selinos_controlled_et_dyn_relative_m0_start(vka_t *vka, vspace_t *vspace)
{
    ...
    _Static_assert(sizeof(seL4_UserContext) / sizeof(seL4_Word) ==
                       SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_X86_64_CONTEXT_WORDS,
                   "Phase 85 requires complete pinned x86_64 context");
    if (vka == NULL || vspace == NULL) {
        return false;
    }
    image = selinos_controlled_et_dyn_relative_m0_fixture(&image_bytes);
    if (selinos_controlled_et_dyn_relative_m0_validate_fixture(image, image_bytes,
    ...
    if (vka_alloc_endpoint(vka, &fault_endpoint) != seL4_NoError ||
        ...
        vka_alloc_cnode_object(vka,
                               SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
        ...
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_CNODE_SELF_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_cnode.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_NOTIFICATION_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_notification.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_IPC_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_ipc_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Mint(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_FAULT_ENDPOINT_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, fault_endpoint.cptr,
                        seL4_WordBits, seL4_AllRights,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FAULT_BADGE) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_ENTRY_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_entry_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_SECOND_RX_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_second_rx_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_STACK_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_stack_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_DATA_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
                        seL4_CapInitThreadCNode, target_data_frame.cptr,
                        seL4_WordBits, seL4_AllRights) != seL4_NoError ||
        seL4_CNode_Copy(target_cnode.cptr,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_TARGET_RO_FRAME_SLOT,
                        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_CNODE_SLOT_BITS,
    ...
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_PAGE_BYTES; ++index) {
        ((volatile uint8_t *)root_entry_mapping)[index] = 0u;
    }
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_ENTRY_PAYLOAD_BYTES;
         ++index) {
        ((volatile uint8_t *)root_entry_mapping)[index] =
            image[SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_ENTRY_LOAD_OFFSET + index];
    }
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_ENTRY_PAYLOAD_BYTES;
         ++index) {
    ...
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_SECOND_RX_PAYLOAD_BYTES;
         ++index) {
        ((volatile uint8_t *)root_second_rx_mapping)[index] =
            image[SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_SECOND_RX_LOAD_OFFSET + index];
    }
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_SECOND_RX_PAYLOAD_BYTES;
         ++index) {
    ...
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RO_INITIALIZED_BYTES;
         ++index) {
        ((volatile uint8_t *)root_ro_mapping)[index] =
            image[SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RO_LOAD_OFFSET + index];
    }
    for (index = 0u; index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RO_INITIALIZED_BYTES;
         ++index) {
    ...
    for (index = 0u;
         index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_DATA_INITIALIZED_BYTES; ++index) {
        ((volatile uint8_t *)root_data_mapping)[index] =
            image[SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_DATA_LOAD_OFFSET + index];
    }
    for (index = 0u;
         index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_DATA_INITIALIZED_BYTES; ++index) {
    ...
    for (index = SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_BSS_OFFSET;
         index < SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_DATA_MEMORY_BYTES; ++index) {
    ...
    if (summary.rela_address != 0x2100u || summary.rela_size != 24u ||
        ...
        *((volatile uint64_t *)root_data_mapping +
          SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RELA_DESTINATION_OFFSET / sizeof(uint64_t)) !=
            0u) {
        return false;
    }
    *((volatile uint64_t *)root_data_mapping +
      SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RELA_DESTINATION_OFFSET / sizeof(uint64_t)) =
        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_LOAD_BASE +
        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RELA_ADDEND;
    if (*((volatile uint64_t *)root_data_mapping +
          SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_RELA_DESTINATION_OFFSET / sizeof(uint64_t)) !=
        SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_SECOND_RX_VADDR) {
        return false;
    }
    ...
    if (sel4utils_map_page(vka, target_vspace_root.cptr, target_ipc_frame.cptr,
                           (void *)SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_IPC_BUFFER_VADDR,
        ...
        sel4utils_map_page_with_attributes(
            ...
            (void *)SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_STACK_VADDR,
            read_write,
        ...
        sel4utils_map_page_with_attributes(
            ...
            (void *)SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_ENTRY_VADDR,
            read_only, seL4_X86_Default_VMAttributes,
            paging_objects, &paging_object_count) != seL4_NoError ||
        sel4utils_map_page_with_attributes(
            vka, target_vspace_root.cptr, target_second_rx_frame.cptr,
            (void *)SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_SECOND_RX_VADDR,
            read_only, seL4_X86_Default_VMAttributes,
            paging_objects, &paging_object_count) != seL4_NoError ||
        sel4utils_map_page_with_attributes(
            vka, target_vspace_root.cptr, target_ro_frame.cptr,
            (void *)SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_RO_VADDR,
            read_only,
            (seL4_X86_VMAttributes)(seL4_X86_Default_VMAttributes |
                                     seL4_X86_ExecuteDisable),
            paging_objects, &paging_object_count) != seL4_NoError ||
        ...
        seL4_TCB_Configure(target_tcb.cptr,
                           ...
                           SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_IPC_BUFFER_VADDR,
                           target_ipc_frame.cptr) != seL4_NoError) {
        return false;
    }

    requested_context.rip = SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_ENTRY_VADDR;
    requested_context.rsp = SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_STACK_POINTER;
    if (seL4_TCB_WriteRegisters(target_tcb.cptr, 0u, 0u,
        ...
        ((const seL4_Word *)&observed_context)
            [SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_X86_64_RFLAGS_WORD] !=
                SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_X86_64_NORMALIZED_RFLAGS ||
    ...
    if (seL4_MessageInfo_get_label(fault_message) != seL4_Fault_UserException ||
        fault_badge != SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FAULT_BADGE ||
        seL4_GetMR(seL4_UserException_FaultIP) !=
            SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_SECOND_RX_VADDR +
                SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_SUCCESS_UD2_OFFSET ||
        seL4_GetMR(seL4_UserException_SP) !=
            SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_FIXED_STACK_POINTER ||
        seL4_GetMR(seL4_UserException_Number) !=
            SELINOS_CONTROLLED_ET_DYN_RELATIVE_M0_INVALID_OPCODE_VECTOR) {
        return false;
    }
    return true;
}
