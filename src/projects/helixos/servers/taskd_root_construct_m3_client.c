...
int main(void)
{
    if (!call_root_construct_m1(SELINOS_ROOT_CONSTRUCT_M1_CONSTRUCTED) ||
        ...
        seL4_DebugPutString("SeLinOS root construction M3 taskd client: M1/M2 provenance transaction rejected.\n");
        for (;;) {
            (void)seL4_Yield();
        }
    }

    seL4_DebugPutString("SeLinOS root construction M3 taskd client: invoking exactly one received-child TCB resume.\n");
    if (seL4_TCB_Resume(SELINOS_ROOT_CONSTRUCT_M3_CHILD_TCB_SLOT) != seL4_NoError) {
        seL4_DebugPutString("SeLinOS root construction M3 taskd client: received-child TCB resume rejected.\n");
        for (;;) {
            (void)seL4_Yield();
        }
    }
    seL4_DebugPutString("SeLinOS root construction M3 taskd client: one received-child TCB resume returned success.\n");
    for (;;) {
        (void)seL4_Yield();
    }
}
