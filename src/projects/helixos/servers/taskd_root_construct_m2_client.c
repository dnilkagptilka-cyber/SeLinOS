...
int main(void)
{
    if (!call_root_construct_m1(SELINOS_ROOT_CONSTRUCT_M1_CONSTRUCTED) ||
        ...
        seL4_DebugPutString("SeLinOS root construction M2 taskd client: M1 construction or duplicate reply rejected.\n");
        for (;;) {
            (void)seL4_Yield();
        }
    }
    if (!call_root_construct_m2_lease(SELINOS_ROOT_CONSTRUCT_M2_LEASE_GRANTED,
                                      SELINOS_ROOT_CONSTRUCT_M2_GRANTED_CAPS)) {
        seL4_DebugPutString("SeLinOS root construction M2 taskd client: provenance lease reply rejected.\n");
        for (;;) {
            (void)seL4_Yield();
        }
    }
    if (!call_root_construct_m2_lease(SELINOS_ROOT_CONSTRUCT_M2_REJECTED,
                                      SELINOS_ROOT_CONSTRUCT_M2_REJECTED_CAPS)) {
        seL4_DebugPutString("SeLinOS root construction M2 taskd client: duplicate lease refusal rejected.\n");
        for (;;) {
            (void)seL4_Yield();
        }
    }

    seL4_DebugPutString("SeLinOS root construction M2 taskd client: one provenance TCB cap received and not exercised; duplicate refused.\n");
    for (;;) {
        (void)seL4_Yield();
    }
}
